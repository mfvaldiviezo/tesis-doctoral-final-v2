...
from resco_benchmark.agents.policy.SWOKS.network_utils import *
from resco_benchmark.agents.policy.SWOKS.ssmask_utils import MultitaskMaskConv2d
from resco_benchmark.agents.policy.SWOKS.ssmask_utils import MultitaskMaskLinear
...
class CustomConvBody(nn.Module):
    def __init__(self, in_channels=1):
        ...
        self.conv1 = layer_init(
            nn.Conv2d(in_channels, 3, kernel_size=(6, 7), stride=(1, 3))
        )
...
class NatureConvBodySS(nn.Module):
    def __init__(self, in_channels=1, num_tasks=8, discrete=True, state_dim=None):
        super(NatureConvBodySS, self).__init__()
        # TODO don't hardcode these
        print("State dim:", state_dim)
        if state_dim == (1, 11, 5):
            flat = 128
        elif state_dim == (1, 18, 5):
            flat = 240
        elif state_dim == (1, 17, 5):
            flat = 224
        elif state_dim == (1, 14, 5):
            flat = 176
        else:
            raise NotImplementedError("State dim not supported")
        self.feature_dim = 16
        ...
        self.conv2 = MultitaskMaskConv2d(
            2, 4, kernel_size=2, stride=1, num_tasks=num_tasks, discrete=discrete
        )
        self.conv3 = MultitaskMaskConv2d(
            4, 8, kernel_size=2, stride=1, num_tasks=num_tasks, discrete=discrete
        )
        self.fc4 = MultitaskMaskLinear(
            flat, self.feature_dim, num_tasks=num_tasks, discrete=discrete
        )
...
class NatureConvBodySSmngrd(nn.Module):
    def __init__(self, in_channels=1, num_tasks=8, discrete=False):
        ...
        self.conv2 = MultitaskMaskConv2d(
            16, 32, kernel_size=2, stride=1, num_tasks=num_tasks, discrete=discrete
        )
        self.conv3 = MultitaskMaskConv2d(
            32, 64, kernel_size=2, stride=1, num_tasks=num_tasks, discrete=discrete
        )
        self.fc4 = MultitaskMaskLinear(
            64, self.feature_dim, num_tasks=num_tasks, discrete=discrete
        )
...
class DummyBody_CL(nn.Module):
    def __init__(self, state_dim, task_label_dim=None):
        ...
        self.feature_dim = (
            state_dim  # + (0 if task_label_dim is None else task_label_dim)
        )
...
class FCBody_CL(nn.Module):  # fcbody for continual learning setup
    def __init__(
        self, state_dim, task_label_dim=None, hidden_units=(64, 64), gate=F.relu
    ):
