...
class MultitaskMaskLinear(nn.Linear):
        ...
        **kwargs,
        ...
        if self.new_mask_type == NEW_MASK_LINEAR_COMB:
            self.betas = nn.Parameter(
                torch.zeros(num_tasks, num_tasks).type(torch.float32)
            )
    ...
    def set_task(self, task, new_task=False):
        ...
        if self.new_mask_type == NEW_MASK_LINEAR_COMB and new_task:
            if task > 0:
                k = task + 1
                self.betas.data[task, 0:k] = 1.0 / k
                # print(self.betas)
...
class MultitaskMaskLinearSparse(nn.Linear):
    def __init__(
        ...
        **kwargs,
        ...
        # subnet class
        self._subnet_class = (
            GetSubnetSparseDiscrete if discrete else GetSubnetSparseContinuous
        )
...
def set_model_task(model, task, verbose=False, new_task=False):
    for n, m in model.named_modules():
        if (
            isinstance(m, MultitaskMaskLinear)
            or isinstance(m, MultitaskMaskLinearSparse)
            or isinstance(m, MultitaskMaskConv2d)
            # or isinstance(m, MultitaskMaskConv2dSparse)
        ):
            if verbose:
...
def consolidate_mask(model):
    for n, m in model.named_modules():
        if isinstance(m, MultitaskMaskLinear) or isinstance(
            m, MultitaskMaskLinearSparse
        ):
            m.consolidate_mask()
...
class MultitaskFC(nn.Module):
    def __init__(self, hidden_size, num_tasks):
        ...
        self.model = nn.Sequential(
            ...
            nn.ReLU(),
            MultitaskMaskLinear(
                hidden_size, hidden_size, num_tasks=num_tasks, bias=False
            ),
            nn.ReLU(),
