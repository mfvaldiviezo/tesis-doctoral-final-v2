...
class ActorCriticNet(nn.Module):
    def __init__(self, state_dim, action_dim, phi_body, actor_body, critic_body):
        ...
        if actor_body is None:
            actor_body = DummyBody(phi_body.feature_dim + 1)
        if critic_body is None:
            critic_body = DummyBody(phi_body.feature_dim + 1)
...
class DeterministicActorCriticNet(nn.Module, BaseNet):
    def __init__(
        ...
        self.network = ActorCriticNet(
            state_dim, action_dim, phi_body, actor_body, critic_body
        )
        self.actor_opt = actor_opt_fn(
            self.network.actor_params + self.network.phi_params
        )
        self.critic_opt = critic_opt_fn(
            self.network.critic_params + self.network.phi_params
        )
...
class GaussianActorCriticNet(nn.Module, BaseNet):
    def __init__(
        self, state_dim, action_dim, phi_body=None, actor_body=None, critic_body=None
    ):
...
#             state_dim, action_dim, phi_body, actor_body, critic_body
#         )
#         from resco_benchmark.agents.policy.SWOKS.ssmask_utils import set_model_task
#
...
class ActorCriticNetMultiHead(nn.Module):
    def __init__(
        self, state_dim, action_dim, phi_body, actor_body, critic_body, num_tasks
    ):
        ...
        for i in range(num_tasks):
            self.actor_params += list(
                (getattr(self, "fc_action_head_{0}".format(i))).parameters()
            )
            self.critic_params += list(
                (getattr(self, "fc_critic_head_{0}".format(i))).parameters()
            )
...
class CategoricalActorCriticNet_CL_MultiHead(nn.Module, BaseNet):
    def __init__(
        ...
        self.network = ActorCriticNetMultiHead(
            state_dim, action_dim, phi_body, actor_body, critic_body, num_tasks
        )
    ...
    def predict(self, obs, action=None, task_label=None, return_layer_output=False):
        obs = tensor(obs).to(cfg.device)
        if task_label is not None and not isinstance(task_label, torch.Tensor):
            task_label = tensor(task_label).to(cfg.device)
        ...
        phi_a, out = self.network.actor_body(
            phi, None, return_layer_output, "network.actor_body"
        )
        phi_v, out = self.network.critic_body(
            phi, None, return_layer_output, "network.critic_body"
        )
...
class QNetMultiHead(nn.Module):
    def __init__(self, state_dim, action_dim, phi_body, critic_body, num_tasks):
        super(QNetMultiHead, self).__init__()
        if phi_body is None:
            phi_body = DummyBody(state_dim)
        if critic_body is None:
            critic_body = DummyBody(phi_body.feature_dim)
        ...
        for i in range(num_tasks):
            setattr(
                self,
                "fc_critic_head_{0}".format(i),
                layer_init(nn.Linear(critic_body.feature_dim, action_dim), 1e-3),
            )
...
class QNet_CL_MultiHead(nn.Module, BaseNet):
    def __init__(
        self,
        state_dim,
        action_dim,
        ...
        super(QNet_CL_MultiHead, self).__init__()
        self.network = QNetMultiHead(
            state_dim, action_dim, phi_body, critic_body, num_tasks
        )
        self.task_label_dim = task_label_dim
        self.task_mapper = []
        from resco_benchmark.agents.policy.SWOKS.ssmask_utils import set_model_task
