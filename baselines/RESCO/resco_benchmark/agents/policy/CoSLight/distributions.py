...
class Categorical_Topk(nn.Module):
    ...
    def forward(self, x, available_actions=None):
        x = self.linear(x)
        # Sanitize NaN/Inf that can appear if upstream network weights are corrupted
    ...
    def log_probs(self, actions):
        safe_probs = torch.clamp(self.probs, min=1e-8)
        return torch.gather(torch.log(safe_probs), 0, actions.long())
    ...
    def entropy(self):
        safe_x = torch.nan_to_num(self.x, nan=0.0, posinf=1e6, neginf=-1e6)
        return FixedCategorical_Topk(logits=safe_x).entropy()
