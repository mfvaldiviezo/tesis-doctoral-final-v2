...
from resco_benchmark.agents.policy.CoSLight.transformer import (
    TransformerEncoder,
    TransformerEncoderLayer,
)
from resco_benchmark.agents.policy.CoSLight.frap_net import ModelBody
...
class TransformerEncoderModel(nn.Module):
    def __init__(
        ...
        self._phase_embedding = ModelBody(
            1, cfg.hidden_layer_size, cfg.state_key, device=device
        ).to(device)
        ...
        self.ps_encoding = PositionalEncoding_Emb(
            d_model=hidden_size, dropout=0, max_len=1000, device=device
        )
        ...
        self.transformer_encoder = TransformerEncoder(
            self.encoder_layer, num_layers, norm=nn.LayerNorm(hidden_size)
        )

        # define output layer
        # self.output_layer = nn.Linear(hidden_size, output_size)
        self.output_layer = nn.Linear(hidden_size, cfg.trans_hidden)
    ...
    def forward(
    ...
    ):  # src.shape : T, B, x  T=1 timestep, B=agent number;; src_mask:16,8
        ...
        # add mask
        src_mask = src_mask.repeat(cfg.act_space, 1, 1)
        # if src_mask is None:
        #     max_len = src.shape[0]
        ...
        x = self.transformer_encoder(
            x_pos, src_mask
        )  ### Transformer input S (sequence length, here refers to the number of agents), N (batchsize, here is 1 moment), F (feature dimension)
        #
    ...
    def compute_score(
        self, src, src_mask=None
    ):  # src.shape : T, B, x  This refers to T=1 time step, B=agent number
        T_batch, num_agents = src.shape[0], src.shape[1]
        phase_emb = self._phase_embedding(src.reshape(T_batch * num_agents, -1))
        x = phase_emb.reshape(T_batch, num_agents, -1)
        ...
        src_mask = src_mask.repeat(cfg.act_space, 1, 1)
        h, attn = self.transformer_encoder(
            x_pos_, src_mask, output_att=True
        )  ### Transformer input S (sequence length, here refers to the number of agents), N (batchsize, here is 1 moment), F (feature dimension)
        #
