import torch
from resco_benchmark.agents.policy.CoSLight.r_actor_critic import (
    R_Actor,
    R_Critic,
    R_Actor_Trans,
)
...
class R_MAPPOPolicy_Trans:
    ...
    def __init__(
        self, obs_space, cent_obs_space, act_space, device=torch.device("cpu")
    ):
        ...
        self.actor = R_Actor_Trans(self.obs_space, self.act_space, self.device)
        ...
        # if not cfg.use_trans_critic: ##### pay attention here
        #     if cfg.use_K != 0:
        ...
        # else:
        #     lists_parameters = []
        #     [lists_parameters.extend(list(self.critic.critics[i].parameters())) for i in range(cfg.num_agents)]
    ...
    def get_actions(
        ...
        """
        ####
        if np.random.uniform() > cfg.epsilon:
            deterministic = True
        ...
        values, rnn_states_critic = self.critic(
            actor_features_new, rnn_states_critic, masks
        )
    ...
    def get_values(self, cent_obs, rnn_states_critic, masks):
        ...
        if cfg.use_K == 0:
            ...
            topk_index = torch.topk(atten_scores, k=cfg.use_K, dim=-1)[1]  #   20.16.5
            # #
            batch_indices = (
                torch.arange(cfg.num_agents)[None, :, None]
                .repeat(batch_size_obs, 1, 1)
                .to(self.device)
            )  # 20.16.1
            topk_index = torch.cat((topk_index, batch_indices), dim=-1)

            values, _ = self.critic(
                cent_obs, rnn_states_critic, masks, topk_index.cpu()
            )

        return values

    def get_values_three(
        self,
        cent_obs,
        obs,
        rnn_states_actor,
    ...
    def evaluate_actions(
        ...
        values, _ = self.critic(
            actor_features_batch, rnn_states_critic, masks, backward=True
        )
