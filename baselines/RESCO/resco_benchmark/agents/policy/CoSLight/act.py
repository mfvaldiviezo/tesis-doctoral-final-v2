from .distributions import Bernoulli, Categorical, DiagGaussian, Categorical_Topk
import torch
...
class ACTLayer(nn.Module):
    ...
    def evaluate_actions(self, x, action, available_actions=None, active_masks=None):
        ...
        if self.mixed_action:
            ...
            action_logits = self.action_out(x, available_actions)
            # Create a list with only the logits for each action
            logits_shape = action_logits.logits.shape
            logits = []
            for i in range(logits_shape[0]):
                logits.append(action_logits.logits[i, action[i][0].int()])
...
class ACTLayer_TopK(nn.Module):
    ...
    def __init__(self, action_space, inputs_dim, use_orthogonal, gain):
        super(ACTLayer_TopK, self).__init__()
        ...
        if action_space.__class__.__name__ == "Discrete":
            ...
            self.action_out = Categorical_Topk(
    ...
    def forward(self, x, available_actions=None, deterministic=False):
        ...
        if self.mixed_action:
            ...
            action_logits = self.action_out(x, available_actions)
            actions = (
                self.action_out.mode() if deterministic else self.action_out.sample()
            )
            action_log_probs = self.action_out.log_probs(actions)

            # actions = action_logits.mode() if deterministic else action_logits.sample()
            # # actions = torch.multinomial(action_logits.logits, num_samples=self.args.use_K, replacement=True)
            # action_log_probs = action_logits.log_probs(actions)

        return actions, action_log_probs

    # def log_probs_from_indices(self, probs, sample_indices):
    #     log_probs = torch.gather(torch.log(probs), 0, sample_indices)
    #     return log_probs
    ...
    def evaluate_actions(self, x, action, available_actions=None, active_masks=None):
        ...
        if self.mixed_action:
            ...
            if active_masks is not None:
                dist_entropy = (
                    self.action_out.entropy() * active_masks.squeeze(-1)
                ).sum() / active_masks.sum()
            else:
