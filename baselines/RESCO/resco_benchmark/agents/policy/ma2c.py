...
class MA2C(IndependentAgent):
    def __init__(self, obs_act, sess=None):
    ...
    def save(self):
        if self.saver is not None:
            self.saver.save(self.sess, cfg.log_dir + "agent_" + "checkpoint")
...
class MA2CAgent(Agent):
    def __init__(
        ...
        total_step = cfg.steps

        self.model = MA2CImplementation(n_s, n_a, n_w, n_f, total_step, name, sess)
    ...
    def observe(self, observation, reward, done, info):
        ...
        if self.steps_done % cfg["batch_size"] == 0 or done:
            if done:
                R = self.model.forward(observation, False, "v")
                obs, acts, dones, Rs, Advs = (
                    self.model.trans_buffer.sample_transition(R)
                )
            else:
                R = self.model.forward(observation, False, "v")
...
class MA2CImplementation:
    def __init__(self, n_s, n_a, n_w, n_f, total_step, name, sess):
        self.name = name
        self.sess = sess
        self.reward_clip = cfg["reward_clip"]
        self.reward_norm = cfg["reward_norm"]
        self.n_s = n_s
        self.n_a = n_a
        self.n_f = n_f
        self.n_w = n_w
        self.n_step = cfg["batch_size"]

        # agent_name is needed to differentiate multi-agents
        self.policy = self._init_policy(
            n_s - n_f - n_w, n_a, n_w, n_f, agent_name=name
        )
    ...
    def _init_policy(self, n_s, n_a, n_w, n_f, agent_name=None):
        n_fw = cfg["num_fw"]
        n_ft = cfg["num_ft"]
        n_lstm = cfg["num_lstm"]
        n_fp = cfg["num_fp"]
    ...
    def _init_scheduler(self):
        ...
        beta_init = cfg["entropy_coef_init"]
        beta_decay = cfg["entropy_decay"]
        ...
        if beta_decay == "constant":
            ...
            beta_min = cfg["ENTROPY_COEF_MIN"]
            beta_ratio = cfg["ENTROPY_RATIO"]
