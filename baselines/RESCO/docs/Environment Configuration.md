...
- pair_to_act_map: In shared parameter algorithms such as MPLight agents might be heterogenous in terms of their action space. Therefore, actions which two signals have in common must be remapped to correspond to the same action in each agent. 
  ...
  - pair_to_act_map are defined for each signal ID with the common act index defined by phase_pairs.
