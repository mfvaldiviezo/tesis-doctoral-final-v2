    ...
    def simulator_step(self, n_steps):
        ...
        if n_steps:
            for _ in range(n_steps):
                ...
                # INYECCION LATAM CHAOS
                if "latam" in self.config.sumo_cfg_dir.lower():
                    try:
                        import sys
                        import os
                        tsc_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../../../tsc_framework/src/core"))
                        if tsc_path not in sys.path:
                            sys.path.append(tsc_path)
                        from latam_chaos_manager import LatamChaosManager
